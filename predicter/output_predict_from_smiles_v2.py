...
from predicter import grad_cam_v2, multi_predict
from transformer import get_train_valid_test

import keras
import keras.backend as K

def smi2pred(df_smi, model_path, task_name_list,
            out_dir='tmp',
            shape=[331, 331, 3], layer_name='mixed10',
            grad_threshold=None,#-1.0,
            is_gradcam_plus=False,
            is_TTA=False,
    ...
    ofilename_array = df_smi['ofilename'].values
    smiles_array = df_smi['smiles'].values
    #for index, series in tqdm(df_smi.iterrows()): # df を1行づつループ回す
    for idx in tqdm(range(df_smi.shape[0])): # こうした方が.iterrows()より速いらしい https://kunai-lab.hatenablog.jp/entry/2018/04/08/134924
        ofilename = ofilename_array[idx] # valuesでnumpy.arrayにしてからのindexingならほとんど生アクセスに近い
        smiles = smiles_array[idx]

        # predictするごとに K.clear_session() しないと処理がどんどん遅くなる（実際20件目以降の処理は1枚gradcam画像出すのに1分以上時間掛かってた）ので、毎回モデルファイルロードする
        ...
        try:
            ...
            grad_cam_img = grad_cam_v2.nobranch_multi_grad_cam(model,
                                                                out_grad_cam_dir,
                                                                Path(ofilename).name,
                                                                x,
                                                                None,
                                                                ...
                                                                is_gradcam_plus=is_gradcam_plus,
...
def load_smi_file_to_df(out_dir, ifilename):
    """ 入力ファイルロードしてデータフレームに変換 """
    ...
    with open(ifilename, 'r') as ifs: # 入力ファイルロード
        ...
        for i, line in enumerate(ifs):
            ...
            smiles_list.append(smiles)
    return pd.DataFrame({'ofilename': ofilename_list, 'smiles': smiles_list})

def plot_barh_each_compound(df_pred_score, task_name_list):
    """ 化合物ごとに横棒グラフ作成 """
...
def plot_hist_all_compound(df_pred_score, out_dir):
...
def main():
    ...
    ap.add_argument("-t", "--thema", type=str, required=True, choices=['NPI', 'GAR', 'kinase', 'NPI_GAR', 'SOO'],#['NPI', 'GAR', 'kinase', 'NPI_GAR', 'cut_NPI_1class', 'cut_GAR_2class', 'cut_NPI_1class_GAR_2class'],
                    help="theme codes (ex. 'NPI', 'GAR', 'kinase', 'SOO').")
    ...
    if args['model_type'] == 'class':
        ...
        if args['thema'] == 'NPI' and args['method_category'] == 'Pharmacology' and args['method_kind'] == 'IC50':
            ...
            grad_threshold = [args['grad_threshold']] * assay_num * class_num
            # taskごとに閾値変更する場合
            if args['grad_threshold'] == -1.0:
                grad_threshold_tsv = '/home/aaa00162/jupyterhub/notebook/H3-050/Submit/DL/tuning/output_optuna_class_NPI_cut_task.Pharmacology.represent_IC50_scaffold_roc/predict/test/plot_pr_thresholds_check/df_f1_max.tsv'
                df_f1_max = pd.read_csv(grad_threshold_tsv, sep='\t')
                df_f1_max = df_f1_max.set_index('task_name')
                df_f1_max = df_f1_max.loc[task_name_list,:] # task_name_listに行の順番合わしておく
                df_f1_max[df_f1_max['threshold'] == 1.0] = 0.5 # データ点無く閾値=1.0になる場合は0.5にしておく
                grad_threshold = df_f1_max['threshold']
            print('grad_threshold:', grad_threshold)
        ...
        if args['thema'] == 'GAR' and args['method_category'] == 'Pharmacology' and args['method_kind'] == 'IC50':
            ...
            # taskごとに閾値変更する場合
            if args['grad_threshold'] == -1.0:
                grad_threshold_tsv = '/home/aaa00162/jupyterhub/notebook/H3-050/Submit/DL/experiment/output_class_GAR.Pharmacology.represent_IC50_scaffold_3task/predict/test/plot_pr_thresholds_check/df_f1_max.tsv'
                df_f1_max = pd.read_csv(grad_threshold_tsv, sep='\t')
                df_f1_max = df_f1_max.set_index('task_name')
                df_f1_max = df_f1_max.loc[task_name_list,:] # task_name_listに行の順番合わしておく
                df_f1_max[df_f1_max['threshold'] == 1.0] = 0.5 # データ点無く閾値=1.0になる場合は0.5にしておく
        ...
        if args['thema'] == 'NPI_GAR' and args['method_category'] == 'Pharmacology' and args['method_kind'] == 'IC50':
            ...
            assay_num = 3+16
            class_num = 4+3
            grad_threshold = [args['grad_threshold']] * (3*4 + 16*3)
        ...
        if args['thema'] == 'kinase' and args['method_category'] == 'Pharmacology' and args['method_kind'] == 'IC50':
            ...
            # taskごとに閾値変更する場合
            if args['grad_threshold'] == -1.0:
                grad_threshold_tsv = '/home/aaa00162/jupyterhub/notebook/H3-050/Submit/DL/experiment/output_class_kinase_result_IC50_scaffold/predict/test/plot_pr_thresholds_check/df_f1_max.tsv'
                df_f1_max = pd.read_csv(grad_threshold_tsv, sep='\t')
                df_f1_max = df_f1_max.set_index('task_name')
                df_f1_max = df_f1_max.loc[task_name_list,:] # task_name_listに行の順番合わしておく
                df_f1_max[df_f1_max['threshold'] == 1.0] = 0.5 # データ点無く閾値=1.0になる場合は0.5にしておく
        ...
        if args['thema'] == 'SOO' and args['method_category'] == 'Pharmacology' and args['method_kind'] == 'IC50':
            # 列の順番が濃度ごとに並んでるので、Assayごとに並び替える
            # ついでに不要なラベル列（<=0.01μM, <=0.001μMとか）削除
            task_name_list = ['CDMF_CA25_PUMA_POTENCY_BACKSUB.AC1.4.CD_SARCOMERE.BKG_SUB_METHOD_BLEBISTATIN<=10um',
                              'CDMF_CA25_PUMA_POTENCY_BACKSUB.AC1.4.CD_SARCOMERE.BKG_SUB_METHOD_BLEBISTATIN<=1um',
                              'CDMF_CA25_PUMA_POTENCY_BACKSUB.AC1.4.CD_SARCOMERE.BKG_SUB_METHOD_BLEBISTATIN<=0.1um',
                              'SKS_CA25_PUMA_POTENCY_BACKSUB.AC1.4.SK_SLOW_SARCOMERE.BKG_SUB_METHOD_BLEBISTATIN<=10um',
                              'SKS_CA25_PUMA_POTENCY_BACKSUB.AC1.4.SK_SLOW_SARCOMERE.BKG_SUB_METHOD_BLEBISTATIN<=1um',
                              'SKS_CA25_PUMA_POTENCY_BACKSUB.AC1.4.SK_SLOW_SARCOMERE.BKG_SUB_METHOD_BLEBISTATIN<=0.1um',
                              'SK_MYOFIBRIL_CA25_PUMA_POTENCY.AC1.4.SK_SARCOMERE<=10um',
                              'SK_MYOFIBRIL_CA25_PUMA_POTENCY.AC1.4.SK_SARCOMERE<=1um',
                              'SK_MYOFIBRIL_CA25_PUMA_POTENCY.AC1.4.SK_SARCOMERE<=0.1um']
            assay_num = 3
            class_num = 5-2
            # SOO_scaffold_6_1.5_2.5 model(InceptionV3)
            model_path = '/home/aaa00162/jupyterhub/notebook/H3-050/Submit/DL/experiment/output_class_SOO_3task_IC50_scaffold_aaa00164_data/best_model.h5'
            layer_name = 'mixed10'

            grad_threshold = [args['grad_threshold']] * assay_num * class_num
            # taskごとに閾値変更する場合
            if args['grad_threshold'] == -1.0:
                grad_threshold_tsv = '/home/aaa00162/jupyterhub/notebook/H3-050/Submit/DL/experiment/output_class_SOO_3task_IC50_scaffold_aaa00164_data/predict/test/plot_pr_thresholds_check/df_f1_max.tsv'
                df_f1_max = pd.read_csv(grad_threshold_tsv, sep='\t')
                df_f1_max = df_f1_max.set_index('task_name')
                df_f1_max = df_f1_max.loc[task_name_list,:] # task_name_listに行の順番合わしておく
                df_f1_max[df_f1_max['threshold'] == 1.0] = 0.5 # データ点無く閾値=1.0になる場合は0.5にしておく
    ...
    # 入力ファイルロード
    df_smi = load_smi_file_to_df(args['output_dir'], args['smiles_path'])
    # MOEでSMILESを画像に変換してpredict+GradCam
    df_pred_score = smi2pred(df_smi,
                            model_path,
                            task_name_list,
                            out_dir=args['output_dir'],
                            shape=[331, 331, 3], layer_name=layer_name,
                            grad_threshold=grad_threshold,
                            is_gradcam_plus=args['is_gradcam_plus'],
                            is_TTA=args['is_TTA'],
                            run_gradcam_task_idx_list=run_gradcam_task_idx_list
                            )
    # 予測結果0件の時
    if df_pred_score is None:
        ...
        return
    # 化合物ごとに横棒グラフ作成
    plot_barh_each_compound(df_pred_score, task_name_list)
    # 全化合物についてスコアのヒストグラム作成
    plot_hist_all_compound(df_pred_score, args['output_dir'])
    ...
    #                                 #'format':cell_red_format # セルの色を赤にする データバーあると見にくいからやめる
    #                                 }
    #                            )
    for i in range(len(task_name_list)):
        coordinate = ws.cell(row=1,column=i+score_col_start_num).coordinate
        col = coordinate.replace("1", "")
        worksheet.conditional_format(col+'4:'+col+str(df_pred_score.shape[0]+header_num),
                                        {'type':'cell',
                                        'criteria':'>=','value':grad_threshold[i], # grad_threshold[i]以上なら条件適用
                                        'format':workbook.add_format({'font_color':'red'}) # 文字色赤にする
                                        #'format':cell_red_format # セルの色を赤にする データバーあると見にくいからやめる
                                        }
                                    )
