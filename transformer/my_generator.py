...
class MyImageDataGenerator(ImageDataGenerator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if rescale == 1.0 / 255 and random_erasing_maxpixel == 255:
        ...
        self.ricap_use_same_random_value_on_batch =
        # GridMask
    ...
    def random_crop(self, original_img):
        ...
        if (
            original_img.shape[0] < self.random_crop_size[0]
        ...
        augmentation = albumentations.RandomCrop(
            self.random_crop_size[0], original_img.shape[1], p=0.5
        )  # albumentationsでやってみる
        x_crop = augmentation(**data)["image"]
        return ndimage.resize(
            x_crop, height, width
        )  # x_cropの画像サイズはcropしたサイズなのでもとの画像サイズに戻す
    ...
    def ricap(
        self, image_batch, label_batch, beta=0.3, use_same_random_value_on_batch=True
    ):
        """
        ...
        def create_masks(start_xs, start_ys, end_xs, end_ys):
            mask_x = np.logical_and(
                np.arange(image_x).reshape(1, 1, -1, 1)
                >= start_xs.reshape(-1, 1, 1, 1),
                np.arange(image_x).reshape(1, 1, -1, 1) < end_xs.reshape(-1, 1, 1, 1),
            )
            mask_y = np.logical_and(
                np.arange(image_y).reshape(1, -1, 1, 1)
                >= start_ys.reshape(-1, 1, 1, 1),
    ...
    def custom_process(self, batches):
            ...
            aug = my_image.Compose(
                [
                    my_image.RandomTransform(
                        width=batch_x[0].shape[1],
                        height=batch_x[0].shape[0],
                        flip_h=False,
                    ),  # Flip, Scale, Resize, Rotateをまとめて処理。
        ...
        if self.is_grayscale == True:
            aug = my_image.Compose(
                [my_image.RandomCompose([my_image.ToGrayScale(p=1)])]
            )
            x = np.zeros(batch_x.shape)
            for i in range(batch_x.shape[0]):
        ...
        if self.random_erasing_prob > 0:
            x = np.zeros(batch_x.shape)
            for i in range(batch_x.shape[0]):
                x[i] = self.random_erasing(
                    batch_x[i],
                    p=self.random_erasing_prob,
                    max_pic=self.random_erasing_maxpixel,
                )
            batch_x = x

        # GridMask
        if self.gridmask_prob > 0:
            aug = albumentations.Compose(
                [GridMask(num_grid=self.gridmask_num_grid, p=self.gridmask_num_grid), ]
            )
            x = np.zeros(batch_x.shape)
            for i in range(batch_x.shape[0]):
                # rescale済みだから255.0かけないとおかしくなる
    ...
    def flow_from_dataframe(
        ...
        batches = super().flow_from_dataframe(
            dataframe,
            directory,
            x_col,
            y_col,
...
def randaugment_generator(gen, N=None, M=None, rescale=1.0 / 255.0):
    ...
    while True:
        ...
        batch_x = np.array(
            [np.asarray(img) * rescale for img in batch_img]
        )  # PIL->numpy
