...
class Objective(object):
    ...
    def __init__(self
                  ...
                  , choice_model=['VGG16','ResNet50','InceptionV3','Xception','InceptionResNetV2','NASNetLarge','SEResNet','SEInceptionV3','SEInceptionResNetV2','SEDenseNet'] # Fine-tuning する学習済みモデル ,'SEResNext' はエラーになる
        ...
        if self.callback_save_model == True:
            ...
            if self.gpu_count == 1:
                if self.return_val_loss:
                    cb.append(keras.callbacks.ModelCheckpoint(filepath=os.path.join(weight_dir, str(trial.trial_id-1)+'.h5'), monitor='val_loss', save_best_only=True, verbose=1))
                else:
                    cb.append(keras.callbacks.ModelCheckpoint(filepath=os.path.join(weight_dir, str(trial.trial_id-1)+'.h5'), monitor='val_'+self.metrics[0], save_best_only=True, verbose=1))
            else:
