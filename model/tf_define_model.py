...
def get_imagenet_model(
    ...
    if not os.path.exists(model_path):
        ...
        if (
            choice_model.lower() == "vgg16"
        ):  # trainable == 15 img_rows=224, img_cols=224, channels=3
            model = keras.applications.vgg16.VGG16(
                weights=weights, include_top=is_include_top, input_tensor=input_tensor
            )

        elif (
            choice_model.lower() == "resnet50"
        ):  # trainable == 164 img_rows=224, img_cols=224, channels=3
            ...
            choice_model.lower() == "inceptionv3"
        ):  # trainable == 249 img_rows=299, img_cols=299, channels=3
            model = keras.applications.inception_v3.InceptionV3(
                weights=weights, include_top=is_include_top, input_tensor=input_tensor
            )

        elif (
            choice_model.lower() == "xception"
        ):  # trainable == 116 img_rows=299, img_cols=299, channels=3
            model = keras.applications.xception.Xception(
                weights=weights, include_top=is_include_top, input_tensor=input_tensor
            )

        elif (
            choice_model.lower() == "inceptionresnetv2"
        ):  # trainable == 761 img_rows=299, img_cols=299, channels=3
        ...
        if (int(keras_ver[0]) == 2) & (int(keras_ver[1]) < 2):  # kerasのバージョン判定
            ...
            if choice_model.lower() == "mobilenet":
                ...
                model = keras.models.load_model(
                    model_path,
                    custom_objects={"relu6": keras.applications.mobilenetv2.relu6},
                )
        else:
...
def get_SENet_model(
    ...
    if not os.path.exists(model_path):
        if choice_model.lower() == "seresnet":
            ...
            if seresnet_num == 18:
                model = se_resnet.SEResNet18(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )  # , pooling='avg')
            elif seresnet_num == 34:
                model = se_resnet.SEResNet34(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )  # , pooling='avg')
            elif seresnet_num == 50:
                model = se_resnet.SEResNet50(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )  # , pooling='avg')
            elif seresnet_num == 101:
                model = se_resnet.SEResNet101(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )  # , pooling='avg')
            elif seresnet_num == 154:
                model = se_resnet.SEResNet154(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )  # , pooling='avg')
        ...
        if choice_model.lower() == "seinceptionv3":
            model = se_inception_v3.SEInceptionV3(
                weights=weights, include_top=is_include_top, input_tensor=input_tensor
            )  # , pooling='avg')

        if choice_model.lower() == "seinceptionresnetv2":
            model = se_inception_resnet_v2.SEInceptionResNetV2(
                weights=weights, include_top=is_include_top, input_tensor=input_tensor
            )  # , pooling='avg')
        ...
        if choice_model.lower() == "sedensenetimagenet":
            ...
            if sedensenet_num == 121:
                model = se_densenet.SEDenseNetImageNet121(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )
            elif sedensenet_num == 161:
                model = se_densenet.SEDenseNetImageNet161(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )
            elif sedensenet_num == 169:
                model = se_densenet.SEDenseNetImageNet169(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )
            elif sedensenet_num == 201:
                model = se_densenet.SEDenseNetImageNet201(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )
            elif sedensenet_num == 264:
                model = se_densenet.SEDenseNetImageNet264(
                    weights=weights,
                    include_top=is_include_top,
                    input_tensor=input_tensor,
                )
        ...
        if choice_model.lower() == "seresnext":
            ...
            if seresnext_num == 50:
                model = se_resnext.SEResNextImageNet(
                    weights=weights,
                    depth=[3, 4, 6, 3],
                    include_top=is_include_top,
                    input_tensor=input_tensor,
...
def get_WideResNet_model(
    ...
    if include_top == True:
        ...
        model = wrn_top_false.create_wide_residual_network(
            (img_rows, img_cols, channels),
            nb_classes=num_classes,
            N=wrn_N,
            k=wrn_k,
...
def get_OctConv_WideResNet_model(
    alpha=0.25, img_rows=32, img_cols=32, channels=3, wrn_N=4, wrn_k=10
):
    """
    ...
    from oct_conv2d import (
        OctConv2D,
    )  # load_model(… custom_objects={'OctConv2D':OctConv2D}, compile=False) が必要
    import models_include_top_false as oct_wrn_top_false  # 出力層消したの

    print("oct_conv_alpha, wrn_N, wrn_k =", alpha, wrn_N, wrn_k)
    model = oct_wrn_top_false.create_octconv_wide_resnet(
        alpha=alpha, N=wrn_N, k=wrn_k, input=(img_rows, img_cols, channels)
    )
    return model
...
def get_EfficientNet_model(
    ...
    model_path = os.path.join(
        output_dir, "EfficientNetB" + str(efficientnet_num) + ".h5"
    )
    os.makedirs(output_dir, exist_ok=True)
    ...
    if efficientnet_num == 0:
        model = EfficientNetB0(
            weights=weights, include_top=is_include_top, input_shape=input_shape
        )
    elif efficientnet_num == 1:
...
def get_Pelee_net(
    input_shape=(224, 224, 3), include_top=True, use_stem_block=True, num_classes=10
):
    """
    ...
    if include_top == True:
        model = pelee_net_keras.PeleeNet(
...
def get_attention_ptmodel(
    num_classes, activation, base_pretrained_model=None, base_model_trainable=False
):
    """
    ...
    if base_pretrained_model is None:
        ...
        base_pretrained_model = keras.applications.inception_v3.InceptionV3(
            input_tensor=in_lay, include_top=False, weights="imagenet"
        )
        pt_features = base_pretrained_model(in_lay)
    else:
        shape = (
            base_pretrained_model.input_shape[1],
            base_pretrained_model.input_shape[2],
            base_pretrained_model.input_shape[3],
        )
    ...
    attn_layer = keras.layers.Conv2D(
        64, kernel_size=(1, 1), padding="same", activation="relu"
    )(keras.layers.Dropout(0.5)(bn_features))
    attn_layer = keras.layers.Conv2D(
    ...
    gap = keras.layers.Lambda(lambda x: x[0] / x[1], name="RescaleGAP")(
        [gap_features, gap_mask]
    )
    gap_dr = keras.layers.Dropout(0.25)(gap)
    dr_steps = keras.layers.Dropout(0.25)(
        keras.layers.Dense(128, activation="relu")(gap_dr)
    )
...
def show_attention_layer(
    retina_model, Xs: np.ndarray, ys=np.array([]), output_dir=None
):
    """
    ...
    attn_func = keras.backend.function(
        inputs=[retina_model.get_input_at(0), keras.backend.learning_phase()],
...
def create_kuzushiji_best_network(shape, num_classes, l2=1e-4) -> tf.keras.models.Model:
    ...
    inputs = x = tf.keras.layers.Input(
        (shape[0], shape[1], shape[2])
    )  # Grad-CAMで入力層のサイズ使うから指定する
...
def get_adabound(
    ...
    if "adabound" in sys.modules.keys():
        ...
        print(
            "It can not be imported because there is no adabound library.Please obtain the adabound library from 'https://github.com/titu1994/keras-adabound'."
        )
...
def get_optimizers(
    ...
    **kwargs
    ...
    if choice_optim == "sgd":
        ...
        print(
            "sgd_lr sgd_momentum sgd_decay sgd_nesterov =",
            lr,
            momentum,
            decay,
            nesterov,
        ...
        print(
            "rmsprop_lr rmsprop_decay rmsprop_rho =", lr, decay, rmsprop_rho
        )  # , epsilon
        ...
        print(
            "adadelta_lr adadelta_decay adadelta_rho =", lr, decay, adadelta_rho
        )  # , epsilon)
        ...
        print(
            "adam_lr adam_decay beta_1 beta_2, amsgrad =",
            lr,
            decay,
            beta_1,
            beta_2,
        ...
        if lr == 0.0:
            lr = 0.002
        optim = keras.optimizers.Adamax(
            lr=lr, decay=decay, beta_1=beta_1, beta_2=beta_2
        )  # , epsilon=epsilon)
        print(
            "adamax_lr adamax_decay beta_1 beta_2 =", lr, decay, beta_1, beta_2
        )  # , epsilon)
        ...
        print(
            "nadam_lr nadam_schedule_decay beta_1 beta_2 =",
            lr,
            schedule_decay,
            beta_1,
            beta_2,
        ...
        print(
            "adabound_lr adabound_final_lr adabound_decay beta_1 beta_2, amsbound =",
            lr,
            lr * 10,
            decay,
            beta_1,
        ...
        print(
            "radam_lr radam_decay beta_1 beta_2, amsgrad total_steps warmup_proportion min_lr =",
            lr,
            decay,
            beta_1,
            beta_2,
...
def FC_batch_drop(
    ...
    x = keras.layers.Activation(activation, name=name + "_act")(
        x
    )  # BatchNormalizationはDenseと活性化の間
    if dropout_rate > 0:
        x = keras.layers.Dropout(dropout_rate, name=name + "_dropout")(x)
    print(
        "dence dropout is_add_batchnorm kernel_initializer l2_rate =",
        dence,
        dropout_rate,
        is_add_batchnorm,
        kernel_initializer,
...
def get_fine_tuning_model(
    ...
    **kwargs
    ...
    print("fcpool =", fcpool)
    print(
        "pred_kernel_initializer pred_l2_rate =", pred_kernel_initializer, pred_l2_rate
    )
    ...
    if choice_model in [
        ...
        trained_model = get_imagenet_model(
            output_dir,
            choice_model,
            img_rows,
            img_cols,
        ...
        trained_model = get_WideResNet_model(
            img_rows=img_rows,
            img_cols=img_cols,
            channels=channels,
        ...
        trained_model = get_OctConv_WideResNet_model(
            alpha=oct_conv_alpha,
            img_rows=img_rows,
            img_cols=img_cols,
            channels=channels,
            wrn_N=wrn_N,
        ...
        if (img_rows is None) and (img_cols is None) and (channels is None):
            trained_model = get_EfficientNet_model(
                output_dir,
                input_shape=None,
                weights=weights,
        ...
        trained_model = get_Pelee_net(
            input_shape=(img_rows, img_cols, channels), include_top=False
        )
    # print(trained_model.summary())

    # attensionレイヤー付けるor全結合多層にするか
    if fcpool == "attention":
        model = get_attention_ptmodel(
            num_classes,
            activation,
            base_pretrained_model=trained_model,
            base_model_trainable=is_base_model_trainable,
        )
    else:
        ...
        if n_multitask == 1:
            ...
            for n_task in range(n_multitask):
                ...
                # 出力層
                task_x = keras.layers.Dense(
                    multitask_pred_n_node,
                    activation=activation,
                    kernel_initializer=pred_kernel_initializer,
                    kernel_regularizer=keras.regularizers.l2(pred_l2_rate),
                    name="task" + str(n_task) + "_pred",
                )(task_x)
...
def add_label_injection_layer(model, labels: list):
    ...
    output_labels = keras.layers.Lambda(lambda x: tf_labels, name="label_injection")(
        model.input
    )
    label_injection_model = keras.models.Model(
        model.input, [model.output, output_labels]
    )
...
def change_l2_softmax_net(model, alpha=16):
    ...
    x = keras.layers.Lambda(
        lambda x: alpha * keras.backend.l2_normalize(x, axis=-1), name="l2_soft"
    )(
        x
    )  # L2ノルムで割って定数倍
